...
fn sdt_ldr(cpu: &mut Cpu, memory: &mut dyn Memory, rd: u32, addr: u32) -> Cycles {
    let (mut value, wait) = memory.load32(addr & !0x3, AccessType::NonSeq);
    ...
    value = value.rotate_right(8 * (addr % 4));
...
#[must_use]
#[inline(always)]
fn sdt_strb(cpu: &mut Cpu, memory: &mut dyn Memory, rd: u32, addr: u32) -> Cycles {
    let mut value = cpu.registers.read(rd);
...
#[inline(always)]
fn imm(_cpu: &mut Cpu, instr: u32) -> u32 {
    instr & 0xFFF
}
