...
///
/// BX Rd
fn arm_bx(cpu: &mut Cpu, memory: &mut dyn Memory, instr: u32) -> Cycles {
    let dest = cpu.registers.read(instr & 0xF);
...
///
/// B <offset>
fn arm_b(cpu: &mut Cpu, memory: &mut dyn Memory, instr: u32) -> Cycles {
...
///
/// BL <offset>
fn arm_bl(cpu: &mut Cpu, memory: &mut dyn Memory, instr: u32) -> Cycles {
    let offset = (instr & 0xFFFFFF).sign_extend(24).wrapping_shl(2);
    let pc = cpu.registers.read(15);
    let dest = pc.wrapping_add(offset);
...
///
/// MRS Rd, CPSR
pub fn arm_mrs_rc(cpu: &mut Cpu, _memory: &mut dyn Memory, instr: u32) -> Cycles {
...
///
/// MRS Rd, SPSR
pub fn arm_mrs_rs(cpu: &mut Cpu, _memory: &mut dyn Memory, instr: u32) -> Cycles {
    // #NOTE: The PC is not allowed to be a destination register here but I don't check it.
    let rd = (instr >> 12) & 0xf;
...
pub fn arm_msr_ic(cpu: &mut Cpu, memory: &mut dyn Memory, instr: u32) -> Cycles {
    if util::likely!((instr & 0x00010000) == 0) {
        // CPSR_flg
...
pub fn arm_msr_is(cpu: &mut Cpu, memory: &mut dyn Memory, instr: u32) -> Cycles {
    if util::likely!((instr & 0x00010000) == 0) {
        // SPSR_flg
        let src = super::alu::bs::imm_nc(instr);
        let spsr = cpu.registers.read_spsr();
...
pub fn arm_msr_rc(cpu: &mut Cpu, _memory: &mut dyn Memory, instr: u32) -> Cycles {
...
pub fn arm_msr_rs(cpu: &mut Cpu, _memory: &mut dyn Memory, instr: u32) -> Cycles {
    // #NOTE: The PC is not allowed to be an operand register here, but I don't check it.
    let src = cpu.registers.read(instr & 0xf);
    if util::likely!((instr & 0x00010000) == 0) {
        // SPSR_flg
...
/// SWI INSTR
fn arm_swi(cpu: &mut Cpu, memory: &mut dyn Memory, _instr: u32) -> Cycles {
    cpu.exception_internal(CpuException::SWI, memory)
}

/// UNDEFINED INSTR
fn arm_undefined(_cpu: &mut Cpu, _memory: &mut dyn Memory, _instr: u32) -> Cycles {
    todo!("unimplemented")
}

#[allow(dead_code)]
pub const ARM_OPCODE_TABLE: [InstrFunction; 4096] = [
    arm_and_lli,
    arm_and_llr,
    arm_and_lri,
    arm_and_lrr,
